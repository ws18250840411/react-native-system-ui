...
| badge | 优先冻结 | 展示组件，交互/跨端差异少 |
| cell | 优先冻结 | 基础展示/点击结构稳定 |
| count-down | 优先冻结 | 逻辑相对独立，跨端差异少 |
| divider | 优先冻结 | 纯展示组件 |
| empty | 优先冻结 | 纯展示组件 |
| flex | 优先冻结 | 纯布局组件 |
| grid | 优先冻结 | 纯布局组件（组合展示） |
| pagination | 优先冻结 | 逻辑相对独立，交互简单 |
| tag | 优先冻结 | 展示为主，交互简单 |
| water-mark | 优先冻结 | 纯展示为主，逻辑相对独立 |
...
| notify | 复核后冻结 | Overlay 承载与动画/队列管理 |
| progress | 复核后冻结 | 动画/渐变/driver 差异与性能 |
| radio | 复核后冻结 | 组选/交互一致性，细节需测试守护 |
| rate | 复核后冻结 | 触控交互与精度/只读等边界 |
| selector | 复核后冻结 | 选择态/布局/无障碍与多端一致性 |
| sidebar | 复核后冻结 | 滚动/受控联动，交互边界较多 |
