...
将 `theme` 设为 `round-button`，即可渲染圆角按钮样式，适合营销场景。
...
| `closeOnClickOverlay` | 同 `closeOnOverlayPress`（与 react-vant/vant 命名对齐） | `boolean` | `false` |
...
| `onClickCloseIcon` | 点击右上角关闭图标时触发 | `() => void` | - |
