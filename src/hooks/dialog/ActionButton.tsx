...
export const ActionButton = (props: ActionButtonProps) => {
  ...
  return <Pressable accessibilityRole="button" disabled={disabled || loading} style={({ pressed }) => [S.btn, { height: tokens.sizes.actionHeight, opacity: pressed && !disabled && !loading ? 0.8 : 1 }]} onPress={disabled || loading ? undefined : onPress}>{divStyle && <View style={divStyle} />}{loading ? <ActivityIndicator size="small" color={txtColor} /> : renderTextOrNode(text ?? '', { color: txtColor, fontFamily: tokens.typography.fontFamily, fontSize: tokens.typography.actionSize, fontWeight: tokens.typography.actionWeight })}</Pressable>
}
