...
function useControllableValue<T = unknown, P extends object = UseControllableValueProps>(props: P = {} as P, options: UseControllableValueOptions<T> = {}): [T, (value: T, ...args: unknown[]) => void] {
  ...
  const [intVal, setIntVal] = useState<T>(() => { if (ctrl) return val; if (hasProp(props, defaultValuePropName)) return p[defaultValuePropName] as T; return defaultValue as T }); const hRef = useRef(p[trigger]); hRef.current = p[trigger]
  const setVal = useCallback((next: T, ...args: unknown[]) => { if (!ctrl) setIntVal(next); if (typeof hRef.current === 'function') (hRef.current as (v: T, ...r: unknown[]) => void)(next, ...args) }, [ctrl]); return [ctrl ? val : intVal, setVal]
}
