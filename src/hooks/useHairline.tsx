...
export const useHairline = ({ show = true, containerStyle, color, width, defaultPaddingHorizontal = 0 }: UseHairlineOptions) =>
  useMemo(() => {
    if (!show) return null
    const f = StyleSheet.flatten(containerStyle) as { paddingHorizontal?: unknown; paddingLeft?: unknown; paddingRight?: unknown; paddingStart?: unknown; paddingEnd?: unknown } | null
    const ph = isNumber(f?.paddingHorizontal) ? f.paddingHorizontal : undefined
    const r = (a?: unknown, b?: unknown) => isNumber(a) ? a : isNumber(b) ? b : isNumber(ph) ? ph : defaultPaddingHorizontal
