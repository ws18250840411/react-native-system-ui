...
export class OverlayStackStore {
  ...
  mount = (o: OverlayStackMountOptions): OverlayStackEntry => {
    const entry: OverlayStackEntry = { key: ++this.keySeed, zIndex: this.resolveZ(o.zIndex), onClose: o.onClose, closeOnBack: o.closeOnBack, lockScroll: o.lockScroll, type: o.type, meta: o.meta }
    this.entries = [...this.entries, entry]; this.emit(); return entry
  }
  update = (key: number, o: Partial<OverlayStackMountOptions>): OverlayStackEntry | undefined => {
    ...
    this.entries = [...this.entries.slice(0, i), next, ...this.entries.slice(i + 1)]; this.emit(); return next
  }
  ...
  private resolveZ = (v?: number) => {
    ...
    const top = this.peek(); return top ? top.zIndex + this.zStep : this.baseZ
  }
...
const syncBack = () => {
  ...
  if (has && !backSub && BackHandler?.addEventListener) {
    backSub = BackHandler.addEventListener('hardwareBackPress', () => {
      ...
      for (let i = cur.length - 1; i >= 0; i--) { if (cur[i].closeOnBack && cur[i].onClose) { cur[i].onClose!(); return true } }
      return false
    })
    return
  }
