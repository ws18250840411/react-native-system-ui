...
const Host = () => {
  ...
  return React.createElement(View, { testID: String(val) })
}

describe('useReducedMotion', () => {
  it('defaults to false', () => {
    const tree = renderer.create(React.createElement(Host))
    expect(tree.root.findByProps({ testID: 'false' })).toBeTruthy()
  })

  it('getReducedMotion returns boolean', () => {
    expect(typeof getReducedMotion()).toBe('boolean')
  })

  it('_resetForTesting resets state', () => {
    _resetForTesting()
