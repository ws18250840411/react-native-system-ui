...
export const useAnimatedTransition = ({ visible, duration = 300, enterEasing = EASE_OUT, exitEasing = EASE_IN, useNativeDriver: driver = nativeDriverEnabled }: AnimatedTransitionOptions): AnimatedTransitionResult => {
  ...
  const run = useCallback((show: boolean, dur: number, easeIn: (v: number) => number, easeOut: (v: number) => number, nd: boolean) => {
    seqRef.current += 1; const seq = seqRef.current; animRef.current?.stop()
    if (show) setMounted(true)
    if (dur <= 0) { progress.setValue(show ? 1 : 0); if (!show) setMounted(false); return }
  ...
  useEffect(() => { run(visible, reduceMotion ? 0 : duration, enterEasing, exitEasing, driver) }, [visible, reduceMotion, duration, enterEasing, exitEasing, driver, run])
