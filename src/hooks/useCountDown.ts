...
const useCountDown = (options: UseCountDownOptions) => {
  ...
  const reset = useCallback((newT?: number) => { pause(); const n = Math.max(0, typeof newT === 'number' && !Number.isNaN(newT) ? newT : timeRef.current); remainRef.current = n; endTimeRef.current = Date.now() + n; setCurrent(parseTime(n)) }, [pause])
