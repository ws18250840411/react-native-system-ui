...
export const createHairlineView = ({
  left, right, top, bottom, width, color, position = 'bottom', enabled = true, borderRadius,
  ...
  if (position === 'all') {
    ...
    if (isWeb && bw > 0 && bw <= 1)
      return {
        position: 'absolute', top: 0, left: 0, width: '200%', height: '200%', borderWidth: 1,
        ...(color && { borderColor: color }), ...(borderRadius != null && { borderRadius: borderRadius * 2 }),
        transform: [{ scale: 0.5 }], transformOrigin: 'left top', pointerEvents: 'none',
      } as ViewStyle
    return {
      position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, borderWidth: bw,
      ...(color && { borderColor: color }), ...(borderRadius != null && { borderRadius }), pointerEvents: 'none',
    } as ViewStyle
  }

  const bw = width ?? (isWeb ? 1 : StyleSheet.hairlineWidth)
  const s: Record<string, unknown> = { position: 'absolute', pointerEvents: 'none', left, right, top, bottom }
