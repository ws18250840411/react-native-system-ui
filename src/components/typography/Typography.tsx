...
const hasTypographyColorKey = (colors: Record<TypographyType, string>, key: string): key is TypographyType => Object.prototype.hasOwnProperty.call(colors, key)
const isEllipsisObject = (ellipsis?: TypographyTextProps['ellipsis']): ellipsis is EllipsisConfig => isPlainObject(ellipsis)

const TypographyTextBaseInner = React.forwardRef<Text, TypographyTextProps>((props, ref) => {
  const { tokensOverride, children, type: typeProp, color: colorProp, size: sizeProp, level, disabled: disabledProp, delete: deleted, underline, center, strong, ellipsis, style, onPress, ...textProps } = props
  ...
  const handleToggleEllipsis = useCallback(() => {
    ...
    setExpanded(prev => { const next = !prev; ellipsisConfig.onExpand?.(next); return next })
  }, [ellipsisConfig])
...
const TypographyLink = React.forwardRef<Text, TypographyLinkProps>((props, ref) => {
  ...
  const handlePress: TextProps['onPress'] = useCallback(async (event: GestureResponderEvent) => {
    if (onPress) { onPress(event); return }
    if (href) { try { await Linking.openURL(href) } catch {} }
  }, [onPress, href])
