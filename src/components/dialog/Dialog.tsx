...
const DialogImpl: React.FC<DialogProps> = props => {
  ...
  const run = useCallback((action: 'confirm' | 'cancel' | 'close', handler?: () => void) => {
    seqRef.current += 1
    ...
    if (isPromiseLike(result)) {
      void result.then(resolved => { if (resolved === false) return; if (seqRef.current !== seq) return; handler?.() }).catch(() => { if (seqRef.current !== seq) return; handler?.() })
      return
    }
    handler?.()
  }, [])
  const handleCloseIcon = useCallback(() => { onClickCloseIconRef.current?.(); run('close', () => onCloseRef.current?.()) }, [run])
  const handleCancel = useCallback(() => { if (cancelLoadRef.current) return; run('cancel', () => onCancelRef.current?.()) }, [run])
  const handleConfirm = useCallback(() => { if (confirmLoadRef.current) return; run('confirm', () => onConfirmRef.current?.()) }, [run])
  ...
  const animStyle = useMemo(() => ({ transform: [{ scale: scaleAnim }] }), [scaleAnim])
  const roundFooterStyle = useMemo(() => [S.roundFooter, { paddingTop: tokens.spacing.messagePaddingTop, paddingHorizontal: tokens.spacing.messagePaddingHorizontal, paddingBottom: tokens.spacing.roundFooterPadding }], [tokens.spacing.messagePaddingHorizontal, tokens.spacing.messagePaddingTop, tokens.spacing.roundFooterPadding])
  const popupBeforeClose = useCallback(() => { const bc = beforeCloseRef.current; if (!bc) return true; try { return bc('close') } catch { return true } }, [])
  ...
  const popupStyle = useMemo(() => [{ backgroundColor: tokens.colors.background, borderRadius: tokens.sizes.borderRadius, padding: 0 }, widthStyle, style], [style, tokens.colors.background, tokens.sizes.borderRadius, widthStyle])
  return (
    <Popup visible={visible} placement="center" round overlay={overlay} overlayStyle={overlayStyle} overlayTestID={overlayTestID} closeOnBackPress={closeOnBackPress} closeOnPopstate={closeOnPopstate} closeOnClickOverlay={mergedCloseOnOverlay} onClickOverlay={onClickOverlay} beforeClose={popupBeforeClose} onClose={onClose} onClosed={onClosed} contentAnimationStyle={animStyle} style={popupStyle} accessibilityRole={'alertdialog' as any} accessibilityLabel={typeof title === 'string' ? title : undefined} {...rest}>
