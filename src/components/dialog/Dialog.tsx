...
const DialogImpl: React.FC<DialogProps> = props => {
  ...
  const run = useCallback((action: 'confirm' | 'cancel' | 'close', handler?: () => void) => { const c = $.current; c.seq += 1; const seq = c.seq; const bc = c.beforeClose; if (!bc) { handler?.(); return }; let result: ReturnType<NonNullable<typeof beforeClose>>; try { result = bc(action) } catch { handler?.(); return }; if (result === false) return; if (isPromiseLike(result)) { void result.then(resolved => { if (resolved === false || $.current.seq !== seq) return; handler?.() }).catch(() => { if ($.current.seq !== seq) return; handler?.() }); return }; handler?.() }, []); const handleCloseIcon = useCallback(() => { $.current.onClickCloseIcon?.(); run('close', () => $.current.onClose?.()) }, [run]); const handleCancel = useCallback(() => { if ($.current.cancelLoad) return; run('cancel', () => $.current.onCancel?.()) }, [run]); const handleConfirm = useCallback(() => { if ($.current.confirmLoad) return; run('confirm', () => $.current.onConfirm?.()) }, [run])
  ...
  const animStyle = { transform: [{ scale: scaleAnim }] }
  const titleStyles = useMemo(() => hTitle ? { wrap: [S.titleWrap, { paddingTop: hContent ? tokens.spacing.titlePaddingTop : tokens.spacing.titleIsolatedPadding, paddingBottom: hContent ? 0 : tokens.spacing.titleIsolatedPadding, paddingHorizontal: hContent ? tokens.spacing.paddingHorizontal : 0, marginBottom: hContent ? tokens.spacing.titleGap : 0 }], text: [S.title, { color: tokens.colors.title, fontFamily: tokens.typography.fontFamily, fontSize: tokens.typography.titleSize, lineHeight: tokens.typography.titleLineHeight, fontWeight: tokens.typography.titleWeight }, titleStyle] } : null, [hTitle, hContent, titleStyle, tokens])
  const msgStyles = useMemo(() => ({ text: [S.msg, { color: isRound ? tokens.colors.title : tokens.colors.message, fontFamily: tokens.typography.fontFamily, fontSize: tokens.typography.messageSize, lineHeight: tokens.typography.messageLineHeight, textAlign: messageAlign }, messageStyle], content: !hChildren ? { alignItems: messageAlign === 'center' ? ('center' as const) : messageAlign === 'left' ? ('flex-start' as const) : ('flex-end' as const) } : null, wrap: [S.msgWrap, { paddingTop: hTitle ? tokens.spacing.messagePaddingTop : tokens.spacing.messagePadding, paddingBottom: isRound ? tokens.spacing.roundFooterPadding : tokens.spacing.messagePadding, paddingHorizontal: tokens.spacing.messagePaddingHorizontal }] }), [isRound, messageAlign, messageStyle, hChildren, hTitle, tokens])
  const fStyles = useMemo(() => ({ border: [S.footerBorder, createHairlineView({ position: 'top', color: tokens.colors.divider, left: 0, right: 0, top: 0 })], round: [S.roundFooter, { paddingTop: tokens.spacing.messagePaddingTop, paddingHorizontal: tokens.spacing.messagePaddingHorizontal, paddingBottom: tokens.spacing.roundFooterPadding }] }), [tokens])
  const popupStyle = useMemo(() => { const w = width ? isNumber(width) ? { width } : { width: String(width) as ViewStyle['width'] } : { width: '90%' as ViewStyle['width'], maxWidth: tokens.sizes.maxWidth }; return [{ backgroundColor: tokens.colors.background, borderRadius: tokens.sizes.borderRadius, padding: 0 }, w, style] }, [width, style, tokens])
  const popupBc = useCallback(() => { const bc = $.current.beforeClose; if (!bc) return true; try { return bc('close') } catch { return true } }, [])
  const roundFooterNode = useMemo(() => (hFooterActions && (
    <View style={fStyles.round}>
  ...
  )), [cancelButtonColor, cancelLoad, cancelProps?.disabled, cancelTxt, confirmButtonColor, confirmLoad, confirmProps?.disabled, confirmTxt, handleCancel, handleConfirm, hFooterActions, fStyles.round, showCancelButton, showConfirmButton, tokens.sizes.roundButtonHeight, tokens.spacing.roundFooterGap])
  const defaultFooterNode = useMemo(() => (hFooterActions && (
    <View style={S.footer}>
      <View style={fStyles.border} pointerEvents="none" />
  ...
  )), [cancelButtonColor, cancelLoad, cancelProps?.disabled, cancelTxt, confirmButtonColor, confirmLoad, confirmProps?.disabled, confirmTxt, fStyles.border, handleCancel, handleConfirm, hFooterActions, showCancelButton, showConfirmButton, tokens])
  const footerNode = footer ?? (isRound ? roundFooterNode : defaultFooterNode)
  return (
    <Popup visible={visible} placement="center" round overlay={overlay} overlayStyle={overlayStyle} overlayTestID={overlayTestID} closeOnBackPress={closeOnBackPress} closeOnPopstate={closeOnPopstate} closeOnClickOverlay={closeOnOverlayPress || closeOnClickOverlay} onClickOverlay={onClickOverlay} beforeClose={popupBc} onClose={onClose} onClosed={onClosed} contentAnimationStyle={animStyle} style={popupStyle} accessibilityRole={'alertdialog' as any} accessibilityLabel={typeof title === 'string' ? title : undefined} {...rest}>
      ...
      {titleStyles && <View style={titleStyles.wrap}>{renderTextOrNode(title, titleStyles.text)}</View>}
      {hContent && <View style={[S.content, msgStyles.content, contentStyle]}>
        {hChildren ? children : <View style={msgStyles.wrap}>{renderTextOrNode(message, msgStyles.text)}</View>}
      </View>}
      {footerNode}
    </Popup>
  )
