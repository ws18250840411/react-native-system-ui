...
const PopupImpl: React.FC<PopupProps> = props => {
  const locale = useLocale(); const layoutDir = useDirection(); const { visible, placement: placementProp, position, title, description, tokensOverride, overlay = true, overlayStyle, overlayAccessibilityLabel = locale?.vanPopup?.closeOverlay ?? 'Close overlay', closeOnOverlayPress, closeOnClickOverlay, overlayTestID = 'popup-overlay', closeable = false, closeIcon, closeIconPosition = 'top-right', round, safeArea = false, safeAreaInsetTop = false, safeAreaInsetBottom: safeBottomP, lockScroll = true, destroyOnClose = true, duration = 300, zIndex, closeOnBackPress = false, closeOnPopstate = false, children, beforeClose, onClickOverlay, onClose, onOpen, onOpened, onClosed, stopPropagation = true, style, contentAnimationStyle, ...rest } = props
  ...
  if (!shouldRender) return null; const rz = stackZ ?? zIndex; const safeContent = safeArea
