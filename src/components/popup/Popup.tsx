...
const PopupImpl: React.FC<PopupProps> = props => {
  ...
  const placement = placementProp ?? position ?? 'center'; const closeOvl = closeOnClickOverlay ?? closeOnOverlayPress ?? true; const isCenter = placement === 'center'; const safeAreaInsetBottom = safeBottomP ?? false; const tokens = usePopupTokens(tokensOverride); const reducedMotion = useReducedMotion(); const cbRef = useRef({ onOpened, onClosed, onOpen, onClose, beforeClose, onClickOverlay }); cbRef.current = { onOpened, onClosed, onOpen, onClose, beforeClose, onClickOverlay }
  const ds = useMemo(() => { const shadow = createPlatformShadow(tokens.shadow); const { colors: c, spacing: s, typography: t, layout: l } = tokens; return { popup: { backgroundColor: c.background, padding: s.padding, ...shadow } as ViewStyle, title: { color: c.title, fontFamily: t.fontFamily, fontSize: t.titleSize, fontWeight: t.titleWeight, marginHorizontal: s.descriptionHorizontal, textAlign: 'center' as const }, titleWrap: { marginTop: s.titleTop, marginBottom: s.titleBottom, marginHorizontal: s.descriptionHorizontal, alignItems: 'center' as const }, desc: { color: c.description, fontFamily: t.fontFamily, fontSize: t.descriptionSize, lineHeight: t.descriptionLineHeight }, descWrap: { marginHorizontal: s.descriptionHorizontal, marginBottom: s.descriptionBottom } as ViewStyle, closeBase: { minWidth: s.closeIconSize, minHeight: s.closeIconSize, padding: s.closeIconPadding } as ViewStyle, closeDef: { width: s.closeIconSize, height: s.closeIconSize } as ViewStyle, side: { width: l.sideWidth, maxWidth: l.maxWidth } as ViewStyle, center: { minWidth: l.minWidth, maxWidth: l.centerMaxWidth } as ViewStyle } }, [tokens])
  const [mounted, setMounted] = useState(visible); const [interVis, setInterVis] = useState(visible); const isOpen = visible || interVis; const canCloseOvl = closeOvl && (onClose || beforeClose); const progress = useRef(new Animated.Value(0)).current; const animRef = useRef<Animated.CompositeAnimation | null>(null); const seqRef = useRef(0); const prevVis = useRef(visible); const closingRef = useRef(false); const isV = placement === 'top' || placement === 'bottom'; const isH = placement === 'left' || placement === 'right'; const dir = placement === 'top' || placement === 'left' ? -1 : 1
  const runAnim = useCallback((show: boolean, rm: boolean) => { seqRef.current += 1; const seq = seqRef.current; animRef.current?.stop(); const d = rm ? 0 : duration; const a = Animated.timing(progress, { toValue: show ? 1 : 0, duration: d, easing: show ? EASE_OUT : EASE_IN, useNativeDriver: nativeDriverEnabled, isInteraction: false }); animRef.current = a; a.start(({ finished }) => { if (!finished || seq !== seqRef.current) return; if (show) cbRef.current.onOpened?.(); else { setInterVis(false); if (destroyOnClose) setMounted(false); cbRef.current.onClosed?.() } }) }, [destroyOnClose, duration, progress]); useEffect(() => { if (visible) { setMounted(true); setInterVis(true); runAnim(true, reducedMotion) } else { if (prevVis.current) runAnim(false, reducedMotion) } }, [runAnim, visible, reducedMotion]); useEffect(() => { if (visible && !prevVis.current) cbRef.current.onOpen?.(); prevVis.current = visible }, [visible]); useEffect(() => () => { animRef.current?.stop() }, [])
  const requestClose = useCallback(async (reason: 'close-icon' | 'overlay' | 'close') => { if (closingRef.current) return; closingRef.current = true; try { if (cbRef.current.beforeClose) { const r = await cbRef.current.beforeClose(reason); if (r === false) return }; cbRef.current.onClose?.() } finally { closingRef.current = false } }, []); useEffect(() => { if (!closeOnPopstate) return; return addPopStateListener(() => { if (visible) requestClose('close') }) }, [closeOnPopstate, requestClose, visible]); const handleOvlPress = useCallback(() => { cbRef.current.onClickOverlay?.(); if (closeOvl) requestClose('overlay') }, [requestClose, closeOvl]); const handleClosePress = useCallback(() => requestClose('close-icon'), [requestClose]); const handleEscape = useCallback(() => requestClose('close'), [requestClose])
  const { zIndex: stackZ } = useOverlayStack({ visible: isOpen, onClose: handleEscape, closeOnBack: closeOnBackPress, lockScroll, zIndex, type: 'popup' }); const { overlayRef, overlayProps } = useAriaOverlay({ isOpen, onClose: () => requestClose('overlay'), isDismissable: closeOvl, overlayProps: { ...(Platform.OS === 'android' ? {} : { accessibilityRole: 'dialog' }), accessibilityLiveRegion: 'polite' } }); const { onLayout: ovlOnLayout, ...ovlRest } = overlayProps; const contentProps = useMemo(() => stopPropagation ? { ...ovlRest, onStartShouldSetResponder: CAPTURE } : ovlRest, [ovlRest, stopPropagation]); const cfg = placementConfig[placement]; const radiusStyle = buildRadius(round, placement, tokens.radius.round); const { width: wW, height: wH } = useWindowDimensions(); const dist = isH ? wW : isV ? wH : 0

  const translateT = useMemo(() => { if (isCenter) return null; const out: [number, number] = [dist * dir, 0]; return cfg.axis === 'y' ? { translateY: progress.interpolate({ inputRange: [0, 1], outputRange: out }) } : { translateX: progress.interpolate({ inputRange: [0, 1], outputRange: out }) } }, [cfg.axis, dir, dist, isCenter, progress]); const baseT = useMemo(() => translateT ? [translateT] : [], [translateT]); const animStyle = useMemo(() => { const et = contentAnimationStyle?.transform; const t = Array.isArray(et) ? [...baseT, ...et] : baseT; const s = { ...contentAnimationStyle, transform: t }; if (isCenter) return { ...s, opacity: progress }; if (contentAnimationStyle?.opacity == null) return { ...s, opacity: 1 }; return s }, [baseT, contentAnimationStyle, isCenter, progress]); const [saTopH, setSaTopH] = useState(0); const onSaTop = useCallback((e: LayoutChangeEvent) => setSaTopH(e.nativeEvent.layout.height), []); const shouldRender = mounted || visible; const hidden = !isOpen; const hasHeader = isRenderable(title) || isRenderable(description)
