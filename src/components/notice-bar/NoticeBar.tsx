...
const NoticeBarImpl: React.FC<NoticeBarProps> = props => {
  ...
  ), [handleItemLayout, hasVerticalLoop, isVertical, itemHeight, resolvedColor, restTextProps, textOnLayout, tokens.typography.fontFamily, tokens.typography.fontSize, verticalTrackItems, verticalTranslateY]); const handleContainerLayout = useCallback((event: LayoutChangeEvent) => { setContainerWidthSafe(event.nativeEvent.layout.width) }, [setContainerWidthSafe]); const handleTextLayout = useCallback((event: LayoutChangeEvent) => { setContentWidthSafe(event.nativeEvent.layout.width); textOnLayout?.(event) }, [setContentWidthSafe, textOnLayout]); const handleNodeLayout = useCallback((event: LayoutChangeEvent) => setContentWidthSafe(event.nativeEvent.layout.width), [setContentWidthSafe])
