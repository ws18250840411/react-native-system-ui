...
const NoticeBarImpl: React.FC<NoticeBarProps> = props => {
  ...
  const verticalItems = !isVertical ? [] : items && items.length ? items : (() => { const arr = React.Children.toArray(children); return arr.length ? arr : text !== undefined ? [text] : [] })(); const hasVerticalLoop = !reducedMotion && isVertical && verticalItems.length > 1; const verticalTrackItems = hasVerticalLoop ? [...verticalItems, verticalItems[0]] : verticalItems; const verticalTranslateY = useRef(new Animated.Value(0)).current; const [itemHeight, setItemHeight] = useState(0); const setContentWidthSafe = useCallback((next: number) => { setContentWidth(prev => (Math.abs(prev - next) < 0.5 ? prev : next)) }, []); const setContainerWidthSafe = useCallback((next: number) => { setContainerWidth(prev => (Math.abs(prev - next) < 0.5 ? prev : next)) }, []); const onCloseRef = useRef(onClose); onCloseRef.current = onClose; const onReplayRef = useRef(onReplay); onReplayRef.current = onReplay; const handleClose = useCallback(() => { setVisible(false); onCloseRef.current?.() }, []); const closePress = useAriaPress({ disabled: mode !== 'closeable' || !visible, onPress: handleClose, extraProps: { accessibilityRole: 'button', accessibilityLabel: locale?.vanNoticeBar?.close ?? 'Close' } }); const barPress = useAriaPress({ disabled: !onPress || !visible, onPress, extraProps: onPress ? { accessibilityRole: 'button' } : undefined }); const rightNode = mode === 'closeable' ? <Pressable hitSlop={8} {...closePress.interactionProps}><Close size={16} fill={resolvedColor} color={resolvedColor} /></Pressable> : mode === 'link' ? <Arrow size={16} fill={resolvedColor} color={resolvedColor} /> : rightIcon || null; const hasLeft = isRenderable(leftIcon); const hasRight = Boolean(rightNode); const effectiveContainerWidth = Math.max(0, containerWidth - (hasLeft ? tokens.spacing.sidePadding : 0) - (hasRight ? tokens.spacing.sidePadding : 0)); const shouldScroll = !reducedMotion && !isVertical && !wrapable && (scrollable ?? contentWidth > effectiveContainerWidth)
  useEffect(() => { if (!visible || isVertical) { translateX.stopAnimation(); return }; if (!shouldScroll || contentWidth === 0 || containerWidth === 0 || resolvedSpeed <= 0 || !Number.isFinite(resolvedSpeed)) { translateX.setValue(0); return }; let cancelled = false; const duration = ((contentWidth + containerWidth) / resolvedSpeed) * 1000; const rtlSign = layoutDir === 'rtl' ? -1 : 1; const run = (initial: boolean) => { translateX.setValue(initial ? 0 : rtlSign * containerWidth); Animated.sequence([Animated.delay(resolvedDelay * 1000), Animated.timing(translateX, { toValue: -rtlSign * contentWidth, duration, easing: Easing.linear, useNativeDriver: nativeDriverEnabled, isInteraction: false })]).start(({ finished }) => { if (finished && !cancelled) { onReplayRef.current?.(); run(false) } }) }; run(true); return () => { cancelled = true; translateX.stopAnimation() } }, [shouldScroll, translateX, visible, resolvedDelay, resolvedSpeed, contentWidth, containerWidth, isVertical, layoutDir])
  useEffect(() => { if (!visible) { verticalTranslateY.stopAnimation(); return }; if (!hasVerticalLoop || itemHeight === 0) { verticalTranslateY.setValue(0); return }; const steps: Animated.CompositeAnimation[] = []; for (let i = 1; i <= verticalItems.length; i += 1) { steps.push(Animated.delay(resolvedVerticalInterval), Animated.timing(verticalTranslateY, { toValue: -itemHeight * i, duration: resolvedVerticalDuration, easing: Easing.linear, useNativeDriver: nativeDriverEnabled, isInteraction: false })) }; steps.push(Animated.timing(verticalTranslateY, { toValue: 0, duration: 0, useNativeDriver: nativeDriverEnabled, isInteraction: false })); const loopAnimation = Animated.loop(Animated.sequence(steps)); loopAnimation.start(); return () => { loopAnimation.stop(); verticalTranslateY.stopAnimation() } }, [visible, hasVerticalLoop, itemHeight, resolvedVerticalDuration, resolvedVerticalInterval, verticalItems, verticalTranslateY])
  const handleItemLayout = useCallback((event: LayoutChangeEvent) => { const height = event?.nativeEvent?.layout?.height; if (!height) return; setItemHeight(prev => (prev === 0 || Math.abs(prev - height) >= 0.5 ? height : prev)) }, []); const verticalContentNode = !isVertical || verticalTrackItems.length === 0 ? null : !hasVerticalLoop ? renderTextOrNode(verticalTrackItems[0], [S.text, { color: resolvedColor, fontFamily: tokens.typography.fontFamily, fontSize: tokens.typography.fontSize }], { numberOfLines: 1, ...restTextProps }) : (
  ...
  ); const handleContainerLayout = useCallback((event: LayoutChangeEvent) => { setContainerWidthSafe(event.nativeEvent.layout.width) }, [setContainerWidthSafe]); const handleTextLayout = useCallback((event: LayoutChangeEvent) => { setContentWidthSafe(event.nativeEvent.layout.width); textOnLayout?.(event) }, [setContentWidthSafe, textOnLayout]); const handleNodeLayout = useCallback((event: LayoutChangeEvent) => setContentWidthSafe(event.nativeEvent.layout.width), [setContentWidthSafe])
  ...
  return (
    <Pressable style={[S.ctr, { backgroundColor: resolvedBackground, paddingHorizontal: tokens.spacing.paddingHorizontal, paddingVertical: wrapable ? tokens.spacing.wrapPaddingVertical : tokens.spacing.paddingVertical, minHeight: tokens.layout.minHeight, borderRadius: tokens.layout.radius }, style]} disabled={barPress.states.disabled} {...barPress.interactionProps} {...rest}>
      ...
      <View pointerEvents="none" onLayout={handleContainerLayout} style={[S.content, wrapable && S.contentWrap, hasLeft && { paddingLeft: tokens.spacing.sidePadding }, hasRight && { paddingRight: tokens.spacing.sidePadding }]}>
        {isVertical ? verticalContentNode : shouldScroll ? (isTextContent ? (
...
const S = StyleSheet.create({ ctr: { flexDirection: 'row', alignItems: 'center' }, side: { alignItems: 'center', justifyContent: 'center' }, content: { flex: 1, flexDirection: 'row', overflow: 'hidden' }, text: { flexShrink: 0 }, scrollText: Platform.select({ web: { whiteSpace: 'nowrap', textOverflow: 'clip' }, default: {} }) as unknown as TextStyle, wrapText: { flexWrap: 'wrap', flexShrink: 1 }, contentWrap: { flexDirection: 'column' }, vViewport: { width: '100%', overflow: 'hidden' }, vTrack: { flexDirection: 'column', width: '100%' }, vItem: { width: '100%', justifyContent: 'center' } })
