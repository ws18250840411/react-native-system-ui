...
const AE = 'RNSU_PORTAL_ADD'; const UE = 'RNSU_PORTAL_UPDATE'; const RE = 'RNSU_PORTAL_REMOVE'; const CE = 'RNSU_PORTAL_CLEAR'; const TVE = DeviceEventEmitter || new NativeEventEmitter(); const IW = Platform.OS === 'web'
...
const PortalManagerView = React.forwardRef<PortalManagerHandle, {}>((_, ref) => {
  ...
  if (e.length === 0) return null; return <View pointerEvents="box-none" style={S.pl} collapsable={false}>{e.map(en => <View key={en.key} pointerEvents="box-none" collapsable={false} style={S.pe}>{en.children}</View>)}</View>
})
let ah = 0; let nh = 1; let ngk = 10000
const gm: PortalManager = { mount: (c: React.ReactNode, k?: number) => { if (typeof __DEV__ !== 'undefined' && __DEV__ && ah === 0) console.warn('[Portal] Please mount <PortalHost> or <ConfigProvider> at the root to enable imperative APIs.'); const rk = k ?? ngk++; if (isNumber(k) && k >= ngk) ngk = k + 1; TVE.emit(AE, { key: rk, children: c }); return rk }, update: (k: number, c: React.ReactNode) => { TVE.emit(UE, { key: k, children: c }) }, unmount: (k: number) => { TVE.emit(RE, { key: k }) } }
...
export const portalStore = { clear: () => { TVE.emit(CE) }, hasHosts: () => ah !== 0 }
