...
const SidebarItemImpl: React.FC<SidebarItemProps> = props => {
  const { title, badge, disabled: disabledProp, dot, onClick, textStyle, badgeStyle, contentStyle: _contentStyle, style, index = 0, children: _children, tokensOverride, ...rest } = props
  ...
  return (
    <Pressable {...rest} {...press.interactionProps} style={[tokens.layout.item, { height: tokens.sizing.itemHeight }, style]}>
      <View style={tokens.layout.indicatorWrapper}>{isActive && <View style={indicatorStyle} />}</View>
      <View style={tokens.layout.itemContent}><View style={tokens.layout.titleRow}>{titleNode}{badgeNode}{dotNode}</View></View>
    </Pressable>
  )
}
