...
const TagImpl: React.FC<TagProps> = props => {
  ...
  const ctrStyle = [tokens.layout.container, { backgroundColor: bgClr, paddingHorizontal: szTok.paddingHorizontal, paddingVertical: szTok.paddingVertical, borderRadius: radius }, markR, style]; const label = !isRenderable(children) ? null : renderTextOrNode(children, [{ color: txtClr, fontSize: szTok.fontSize, lineHeight: szTok.lineHeight, fontFamily: tokens.typography.fontFamily, fontWeight: tokens.typography.fontWeight }, textStyle]); const close = closeable && <Pressable accessibilityRole="button" hitSlop={tokens.spacing.closeHitSlop} style={[tokens.layout.close, { marginLeft: tokens.spacing.closeGap }]} onPress={e => { e.stopPropagation?.(); onClose?.() }}>{typeof closeIcon === 'function' ? closeIcon(txtClr, tokens.sizing.closeIconSize) : closeIcon ?? <Close color={txtClr} size={tokens.sizing.closeIconSize} />}</Pressable>; const hlOverlay = plain ? <View style={createHairlineView({ position: 'all', color: borderClr, borderRadius: resRadius })} /> : null; const content = <>{label}{close}{hlOverlay}</>; if (onPress) return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [ctrStyle, pressed && { opacity: tokens.defaults.pressedOpacity }]} {...rest}>{content}</Pressable>; return <View style={ctrStyle} {...rest}>{content}</View>
}
