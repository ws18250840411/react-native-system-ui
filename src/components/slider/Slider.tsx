import { useSlider, useSliderThumb } from '../../internal/aria/slider'
import { isRTL } from '../../internal/aria/utils'
...
import { isFunction, isFiniteNumber, isText } from '../../utils/base'
...
const defaultNumberFormatter =
  typeof Intl !== 'undefined' && isFunction(Intl.NumberFormat)
    ...
    : ({ format: (value: number) => String(value) } as unknown as Intl.NumberFormat)
...
const SliderImpl: React.FC<SliderProps> = props => {
  ...
  const onStateChange = useCallback((values: readonly number[]) => {
    onChangeRef.current?.(formatOutput(values))
  }, [formatOutput])
  const onStateChangeEnd = useCallback((values: readonly number[]) => {
  ...
  const { trackProps } = useSlider(
    { orientation, isDisabled: ariaDisabled, 'aria-label': ariaLabel ?? 'Slider' } as Parameters<typeof useSlider>[0],
    state,
    trackLayout,
    ariaReverse,
  )
  ...
  const onCombinedTrackLayout = useCallback((event: LayoutChangeEvent) => {
  ...
  const onTrack = useCallback((event: GestureResponderEvent) => {
    ...
    if (!state.values.every((_, index) => !state.isThumbDragging(index))) return
    const { locationX, locationY, pageX, pageY } = event.nativeEvent
    const localX = isFiniteNumber(locationX)
      ? locationX
      : (isFiniteNumber(pageX) ? pageX - (trackLayout.x ?? 0) : 0)
    const localY = isFiniteNumber(locationY)
      ? locationY
      : (isFiniteNumber(pageY) ? pageY - (trackLayout.y ?? 0) : 0)
    ...
    const closestThumb = state.values.reduce((best, currentValue, index) => (
      Math.abs(currentValue - value) < Math.abs(state.values[best] - value) ? index : best
    ), 0)
  ...
  const dragStartRef = useRef<boolean[]>([])
  ...
  const enhanceHandlersCacheRef = useRef<WeakMap<object, Map<number, HandlerBag>>>(new WeakMap())
  ...
  const enhanceHandlers = useCallback((handlers: HandlerBag | undefined, index: number) => {
    ...
    const hasAny =
      START_KEYS.some(key => isFunction(handlers[key])) ||
      MOVE_KEYS.some(key => isFunction(handlers[key])) ||
      END_KEYS.some(key => isFunction(handlers[key]))
