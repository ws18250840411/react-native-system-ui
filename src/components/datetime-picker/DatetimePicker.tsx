...
const DatetimePickerImpl: React.FC<DatetimePickerProps> = props => {
  const tokens = useDatetimePickerTokens(); const [popupVisible, setPopupVisible] = useControllableValue<boolean>(props, { defaultValue: false, valuePropName: 'popupVisible', defaultValuePropName: 'defaultPopupVisible', trigger: 'onPopupVisibleChange' }); const closePopup = () => setPopupVisible(false); const { popup, popupVisible: _popupVisible, defaultPopupVisible: _defaultPopupVisible, popupProps, onPopupVisibleChange: _onPopupVisibleChange, onConfirm, onCancel, ...pickerProps } = props; const handleConfirm = (value: Date | string) => { (onConfirm as ((v: Date | string) => void) | undefined)?.(value); if (popup) closePopup() }; const handleCancel = () => { onCancel?.(); if (popup) closePopup() }; const pickerNode = props.type === 'time' ? <TimePicker {...pickerProps as DatetimePickerTimeProps} onConfirm={handleConfirm} onCancel={handleCancel} /> : <DatePicker {...pickerProps as DatetimePickerDateProps} onConfirm={handleConfirm} onCancel={handleCancel} />; return !popup ? pickerNode : <Popup visible={popupVisible} onClose={closePopup} placement={tokens.defaults.popupPlacement} round={tokens.defaults.popupRound} safeAreaInsetBottom={tokens.defaults.popupSafeAreaInsetBottom} {...popupProps}>{pickerNode}</Popup>
}
...
const DatePicker: React.FC<DatetimePickerDateProps> = props => {
  ...
  const handleChange = useCallback((values: (string | number)[]) => { const next = buildDateFromValues(values.map(String)); setCurrentDate(next); onChange?.(next) }, [buildDateFromValues, onChange]); const curDateRef = useRef(currentDate); curDateRef.current = currentDate; const handleConfirm = useCallback(() => onConfirm?.(curDateRef.current), [onConfirm]); return <Picker {...pickerProps} columns={columns} interactionMode={interactionMode} value={pickerValue} onChange={handleChange} onConfirm={handleConfirm} />
}

const TimePicker: React.FC<DatetimePickerTimeProps> = props => {
  const { type: _type, formatter = (_type, value) => value, filter, columnsOrder: _columnsOrder, minHour = 0, maxHour = 23, minMinute = 0, maxMinute = 59, interactionMode = 'freeze', value, defaultValue, onChange, onConfirm, ...pickerProps } = props; const timeRef = useRef<string>(''); const formatTime = useCallback((timeValue?: string) => { const [hour = 0, minute = 0] = (timeValue ?? '').split(':').map(num => parseInt(num, 10)); return `${padZero(clamp(Number.isNaN(hour) ? minHour : hour, minHour, maxHour))}:${padZero(clamp(Number.isNaN(minute) ? minMinute : minute, minMinute, maxMinute))}` }, [maxHour, maxMinute, minHour, minMinute]); const [currentTime, setCurrentTime] = useState(() => { const init = formatTime(value ?? defaultValue); timeRef.current = init; return init }); useEffect(() => { const next = isString(value) ? formatTime(value) : formatTime(timeRef.current); if (next !== timeRef.current) { timeRef.current = next; setCurrentTime(next) } }, [formatTime, value]); const [hourValues, minuteValues] = useMemo(() => { let h = times(maxHour - minHour + 1, i => padZero(minHour + i)); let m = times(maxMinute - minMinute + 1, i => padZero(minMinute + i)); if (filter) { h = filter('hour', h); m = filter('minute', m) }; return [h, m] as [string[], string[]] }, [filter, maxHour, maxMinute, minHour, minMinute]); const columns = useMemo(() => [hourValues.map(v => ({ label: formatter('hour', v), value: v })), minuteValues.map(v => ({ label: formatter('minute', v), value: v }))], [formatter, hourValues, minuteValues]); const handleChange = useCallback((values: (string | number)[]) => { const next = `${String(values[0] ?? hourValues[0])}:${String(values[1] ?? minuteValues[0])}`; timeRef.current = next; setCurrentTime(next); onChange?.(next) }, [hourValues, minuteValues, onChange]); const handleConfirm = useCallback(() => onConfirm?.(timeRef.current), [onConfirm]); return <Picker {...pickerProps} columns={columns} interactionMode={interactionMode} value={currentTime.split(':')} onChange={handleChange} onConfirm={handleConfirm} />
}
