...
import { isFunction, isText } from '../../utils/base'
import { renderTextOrNode } from '../../utils/render'
import { isRenderable } from '../../utils/base'
import { useAutoClose, useLatestRef, useVisibilityLifecycle } from '../../internal/feedback'
...
const NotifyContentImpl: React.FC<NotifyProps> = props => {
  ...
  const contentHeight = barHeight > 0 ? barHeight : tokens.sizing.minHeight; const interactive = closeOnClick || isFunction(onClick); const handlePress = useCallback(() => { callbacksRef.current.onClick?.(); if (closeOnClick) callbacksRef.current.onClose?.() }, [callbacksRef, closeOnClick]); const accessibilityRole = interactive ? 'button' : 'alert'; const pressProps = useAriaPress({ disabled: !interactive, onPress: handlePress, extraProps: { accessibilityRole, accessibilityLiveRegion: 'assertive' } })
