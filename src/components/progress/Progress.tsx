...
export const ProgressFilledTrack: React.FC<{ style?: ViewStyle | ViewStyle[] }> = ({ style }) => {
  ...
  const indicatorSize = useMemo(() => context.animatedValue.interpolate({ inputRange: [0, 100], outputRange: ['0%', '100%'] }), [context.animatedValue])

  const sizeStyle = useMemo(() => context.orientation === 'vertical' ? ({ width: context.height, height: indicatorSize, bottom: 0, left: 0, position: 'absolute' } as unknown as ViewStyle) : ({ height: context.height, width: indicatorSize } as unknown as ViewStyle), [context.height, context.orientation, indicatorSize])

  const baseStyle = [
    context.orientation === 'vertical' ? ({ position: 'absolute', left: 0, bottom: 0 } as unknown as ViewStyle) : context.layoutIndicator,
    { backgroundColor: context.indicatorColor, borderRadius: context.height / 2, ...(context.isGradient && context.gradientColor ? ({ backgroundImage: context.gradientColor } as unknown as ViewStyle) : null) } as unknown as ViewStyle,
    sizeStyle,
...
export const Progress = memo((props: ProgressProps) => {
  ...
  const resolvedPivotBg = pivotColor ?? (isGradient ? inactive ? tokens.colors.track : tokens.colors.indicator : (resolvedIndicatorColor as string))
  ...
  const onPivotLayout = useCallback((e: LayoutChangeEvent) => {
    const width = e.nativeEvent.layout.width
  ...
  const trackStyle = [
    tokens.layout.track,
    orientation === 'vertical' ? ({ width: height, height: '100%', backgroundColor: resolvedTrackColor, borderRadius: height / 2 } as unknown as ViewStyle) : ({ height, backgroundColor: resolvedTrackColor, borderRadius: height / 2 } as ViewStyle),
  ]
  ...
  const pivotNode = useMemo(() => {
    ...
    if (trackW > 0 && pivotW > 0) {
      ...
      if (p1 < p2) {
        transformStyle = { transform: [{ translateX: animatedValue.interpolate({ inputRange: [0, p1, p2, 100], outputRange: [0, 0, trackW - pivotW, trackW - pivotW], extrapolate: 'clamp' }) }] }
      } else {
        transformStyle = { transform: [{ translateX: (trackW - pivotW) / 2 }] }
      }
    }

    return (
      <Animated.View style={[pivotContainerStyle, transformStyle]} pointerEvents="none" onLayout={onPivotLayout}>
        {isText(pivotContent) ? <Text style={[tokens.layout.pivotText, { color: resolvedPivotTextColor, fontSize: tokens.typography.pivotFontSize }, pivotStyle]}>{pivotContent}</Text> : pivotContent}
      </Animated.View>
    )
  }, [
    animatedValue,
  ...
  return (
    <ProgressContext.Provider value={progressContextValue}>
      <View style={style} accessibilityRole="progressbar" accessibilityValue={{ min: 0, max: 100, now: percentage }} {...rest}>
        <View style={trackStyle} onLayout={hasPivot ? onTrackLayout : undefined}>
