...
const SelectorImpl = <V extends SelectorValue>(props: SelectorProps<V>) => {
  const { tokensOverride, options, columns: colP, multiple: multP, showCheckMark: showChkP, disabled: disP, onChange, itemStyle, labelStyle, descriptionStyle, style, ...rest } = props; const tokens = useSelectorTokens(tokensOverride); const columns = colP ?? tokens.defaults.columns ?? 1; const multiple = multP ?? tokens.defaults.multiple; const showCheckMark = showChkP ?? tokens.defaults.showCheckMark; const disabled = Boolean(disP ?? tokens.defaults.disabled); const [value = [], triggerChange] = useControllableValue<V[]>(props, { defaultValue: [] }); const optionsByVal = useMemo(() => { const m = new Map<SelectorValue, (typeof options)[number]>(); for (const o of options) m.set(o.value, o); return m }, [options]); const rCols = Math.max(1, Math.floor(columns)); const basis = `${100 / rCols}%` as `${number}%`; const itemMargin = tokens.spacing.gap / 2; const selectedSet = useMemo(() => new Set(value), [value]); const toggleOption = useCallback((option: (typeof options)[number]) => { if (disabled || option.disabled) return; const active = selectedSet.has(option.value); const nextVal: V[] = multiple ? (active ? value.filter(item => item !== option.value) : [...value, option.value]) : (active ? [] : [option.value]); triggerChange(nextVal, { items: nextVal.map(v => optionsByVal.get(v)).filter(Boolean) as (typeof options)[number][] }) }, [disabled, multiple, optionsByVal, selectedSet, triggerChange, value]); return <View {...rest} style={[tokens.layout.container, { marginHorizontal: -itemMargin, marginVertical: -itemMargin }, style]} accessibilityRole={multiple ? undefined : 'radiogroup'}>{options.map(option => <SelectorOptionItem key={String(option.value)} option={option} active={selectedSet.has(option.value)} disabled={disabled || Boolean(option.disabled)} multiple={multiple} showCheckMark={showCheckMark} basis={basis} itemMargin={itemMargin} tokens={tokens} itemStyle={itemStyle} labelStyle={labelStyle} descriptionStyle={descriptionStyle} onToggle={toggleOption} />)}</View>
}
