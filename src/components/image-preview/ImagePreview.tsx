...
const ImagePreviewImpl = (props: ImagePreviewProps, ref: React.ForwardedRef<ImagePreviewRef>) => {
  const { visible, images = [], startPosition = 0, tokensOverride, lazyRender = false, lazyRenderBuffer = 1, showIndex = true, indexRender, showIndicators = false, closeable = false, closeIcon, closeIconPosition = 'top-right', closeOnlyClickCloseIcon = false, overlay = true, overlayStyle, closeOnBackPress, closeOnPopstate, zIndex, duration, safeAreaInsetTop = true, safeAreaInsetBottom = true, onChange, onClose, onClosed, beforeClose } = props; const { colors, layout, radii, typography, spacing } = useImagePreviewTokens(tokensOverride); const swiperRef = useRef<SwiperInstance>(null); const pendingCloseReason = useRef<ImagePreviewCloseReason>('close'); const touchStartRef = useRef<{ x: number; y: number } | null>(null); const touchMovedRef = useRef(false); const imgLen = images.length; const [activeIndex, setActiveIndex] = useState(() => clampIndex(startPosition, imgLen)); const safeIdx = clampIndex(activeIndex, imgLen); const latestRef = useRef({ images, index: safeIdx, beforeClose, onChange, onClose }); latestRef.current = { images, index: safeIdx, beforeClose, onChange, onClose }; const resolvedImages: ImageSourcePropType[] = useMemo(() => images.map(img => isString(img) ? { uri: img } : img), [images]); useEffect(() => { setActiveIndex(cur => clampIndex(cur, imgLen)) }, [imgLen]); useEffect(() => { if (!visible) return; const next = clampIndex(startPosition, imgLen); setActiveIndex(next); if (typeof requestAnimationFrame !== 'undefined') { const raf = requestAnimationFrame(() => { swiperRef.current?.swipeTo(next, false) }); return () => cancelAnimationFrame(raf) }; swiperRef.current?.swipeTo(next, false) }, [imgLen, startPosition, visible]); const runBeforeClose = async (reason: ImagePreviewCloseReason) => { const { beforeClose: bc, images: imgs, index } = latestRef.current; if (!bc) return true; const res = await bc({ reason, index, image: imgs[index] }); return res !== false }; const executeClose = async (reason: ImagePreviewCloseReason, bypassBeforeClose = false) => { const { onClose: oc, images: imgs, index } = latestRef.current; if (!bypassBeforeClose && !await runBeforeClose(reason)) return; oc?.({ index, image: imgs[index] }) }; const pressableHandlers: PressableHandlers = useMemo(() => { const resetTouch = () => { touchStartRef.current = null; touchMovedRef.current = false }; const startTouch = (x: number, y: number) => { touchStartRef.current = { x, y }; touchMovedRef.current = false }; const moveTouch = (x: number, y: number) => { const start = touchStartRef.current; if (!start) return; const dx = x - start.x; const dy = y - start.y; if (dx * dx + dy * dy >= TOUCH_MOVE_THRESHOLD_SQUARED) touchMovedRef.current = true }; const endTouch = (x: number, y: number) => { const start = touchStartRef.current; const moved = touchMovedRef.current; resetTouch(); if (!start || moved) return; const dx = x - start.x; const dy = y - start.y; if (dx * dx + dy * dy < TOUCH_MOVE_THRESHOLD_SQUARED && !closeOnlyClickCloseIcon) void executeClose('content') }; const handlers: PressableHandlers = { onTouchStart: e => { const { pageX, pageY } = e.nativeEvent; if (pageX != null && pageY != null) startTouch(pageX, pageY) }, onTouchMove: e => { const { pageX, pageY } = e.nativeEvent; if (pageX != null && pageY != null) moveTouch(pageX, pageY) }, onTouchEnd: e => { const { pageX, pageY } = e.nativeEvent; if (pageX != null && pageY != null) endTouch(pageX, pageY) }, onTouchCancel: resetTouch }; if (IS_WEB) { handlers.onMouseDown = e => { const nativeEvent = e.nativeEvent; if (nativeEvent?.pageX != null && nativeEvent?.pageY != null) startTouch(nativeEvent.pageX, nativeEvent.pageY) }; handlers.onMouseMove = e => { const nativeEvent = e.nativeEvent; if (nativeEvent?.buttons !== 1) return; if (nativeEvent?.pageX != null && nativeEvent?.pageY != null) moveTouch(nativeEvent.pageX, nativeEvent.pageY) }; handlers.onMouseUp = e => { const nativeEvent = e.nativeEvent; if (nativeEvent?.pageX != null && nativeEvent?.pageY != null) endTouch(nativeEvent.pageX, nativeEvent.pageY) } }; return handlers }, [closeOnlyClickCloseIcon]); useImperativeHandle(ref, () => ({ swipeTo: (index: number, anim = true) => { const next = clampIndex(index, imgLen); setActiveIndex(next); swiperRef.current?.swipeTo(next, anim) } }), [imgLen]); const handleSwiperChange = (index: number) => { if (latestRef.current.index === index) return; setActiveIndex(index); latestRef.current.onChange?.(index) }; const renderIndex = (current: number, total: number) => !showIndex || total === 0 ? null : (
  ...
  ); const lazyBuffer = lazyRender ? Math.max(0, lazyRenderBuffer | 0) : 0; const renderIndicator = (total: number, current: number) => (<>{renderIndex(current, total)}{showIndicators && total > 1 && <Swiper.PagIndicator total={total} current={current} activeColor={colors.indicatorActive} inactiveColor={colors.indicatorInactive} style={indicatorSpacing} testID="rv-image-preview-indicators" />}</>)
  return (
    <Popup visible={visible} overlay={overlay} overlayStyle={overlayStyle} closeOnOverlayPress={!closeOnlyClickCloseIcon} closeOnBackPress={closeOnBackPress} closeOnPopstate={closeOnPopstate} zIndex={zIndex} duration={duration} closeable={closeable} closeIcon={closeIcon} closeIconPosition={closeIconPosition} stopPropagation={false} round={false} safeAreaInsetTop={safeAreaInsetTop} safeAreaInsetBottom={safeAreaInsetBottom} overlayTestID="rv-image-preview-overlay" style={[S.popup, { backgroundColor: colors.background, padding: layout.popupPadding, borderRadius: layout.popupRadius }]} beforeClose={async reason => { pendingCloseReason.current = reason; return runBeforeClose(reason) }} onClose={() => { void executeClose(pendingCloseReason.current, true) }} onClosed={onClosed}>
          ...
          <Swiper ref={swiperRef} style={S.swiper} initialSwipe={clampIndex(startPosition, imgLen)} loop={false} autoplay={false} touchable={imgLen > 1} indicator={renderIndicator} onChange={handleSwiperChange} testID="rv-image-preview-swiper">
            {resolvedImages.map((source, index) => (
              <Swiper.Item key={index} style={S.slide} testID={`rv-image-preview-slide-${index}`}>
                <ImageSlide source={source} rendered={!lazyRender || Math.abs(index - safeIdx) <= lazyBuffer} pressableHandlers={pressableHandlers} index={index} total={imgLen} />
              </Swiper.Item>
            ))}
          </Swiper>
        )}
