...
const W = StyleSheet.create({ column: { flex: 1 }, option: { justifyContent: 'center', alignItems: 'center' }, grab: { cursor: 'pointer', userSelect: 'none', touchAction: 'none' } as unknown as ViewStyle })
...
const WheelPickerItemInner = <T extends PickerOption>({ item, index, itemHeight, active, disabled, renderItem }: WheelPickerItemProps<T>) => {
  const content = renderItem(item, index, { active, disabled }); return <View style={[W.option, { height: itemHeight }]}>{content}</View>
}
...
const getVelocityBucket = (v: number) => { const abs = Math.abs(v); if (abs > 1.2) return 2; if (abs > 0.6) return 1; return 0 }

const adjustIndex = (idx: number, opts: PickerOption[]) => { const total = opts.length; if (!total) return 0; const i = clamp(idx, 0, total - 1); const next = findEnabledIndex(opts, i); return next >= 0 ? next : i }

const indexToOffset = (idx: number, h: number) => -idx * h

const offsetToIndex = (off: number, h: number, total: number, opts: PickerOption[]) => {
  const minOff = -Math.max(0, total - 1) * h; const offClamped = clamp(off, minOff, 0); let idx = Math.round(-offClamped / h); idx = adjustIndex(idx, opts); const snapOff = indexToOffset(idx, h); return { index: idx, snapOffset: snapOff }
}

const shouldMomentum = (dist: number, dur: number) => dur < 500 && Math.abs(dist) > 8

const momentumTarget = (dist: number, dur: number, currOff: number, h: number, minOff: number) => { const speed = Math.abs(dist / dur); const extra = (speed / 0.0025) * (dist < 0 ? -1 : 1); const tgt = clamp(currOff + extra, minOff, 0); const snapIdx = Math.round(-tgt / h); return indexToOffset(snapIdx, h) }

const WheelPickerInner = <T extends PickerOption,>({ data, selectedIndex, onChange, onInteractStart, onInteractEnd, renderItem, itemHeight, visibleRest, readOnly, indicatorColor, decelerationRate = Platform.select({ ios: 0.9985, android: 0.995, default: 0.995 }) ?? 'normal', scrollEventThrottle = 16, swipeDuration = 300 }: WheelPickerProps<T>) => {
  const isWeb = Platform.OS === 'web'; const listRef = useRef<FlatList<T>>(null), scrollRef = useRef<React.ElementRef<typeof ScrollView>>(null); const spacerHeight = visibleRest * itemHeight; const total = data.length; const maxIdx = Math.max(0, total - 1); const minOff = -maxIdx * itemHeight; const containerHeight = itemHeight * (visibleRest * 2 + 1); const rawSelIdx = clamp(selectedIndex, 0, maxIdx); const enabledSelIdx = findEnabledIndex(data, rawSelIdx); const safeSelIdx = enabledSelIdx >= 0 ? enabledSelIdx : rawSelIdx; const visibleCnt = visibleRest * 2 + 1; const effectiveScrollThrottle = total > visibleCnt * 20 ? 32 : scrollEventThrottle; const webVirtualEnabled = total > visibleCnt * 4
  ...
  const emitIdx = useCallback((offsetY: number, animated: boolean) => { if (readOnly) return; const { index, snapOffset } = offsetToIndex(-offsetY, itemHeight, total, data); const nextOff = -snapOffset; if (Math.abs(nextOff - offsetY) > 0.5) { listRef.current?.scrollToOffset({ offset: nextOff, animated }) }; onChange(index) }, [data, itemHeight, onChange, readOnly, total])
  useEffect(() => { const offset = safeSelIdx * itemHeight; if (isWeb) return; scrollRef.current?.scrollTo({ y: offset, animated: false }) }, [isWeb, itemHeight, safeSelIdx])
  const [webOffset, setWebOffset] = useState(() => indexToOffset(safeSelIdx, itemHeight)); const webOffsetRef = useRef(webOffset), startOffsetRef = useRef(0), startTimeRef = useRef(0); const [webTransition, setWebTransition] = useState(0); const [webVelocityBucket, setWebVelocityBucket] = useState(0); const webVelocityBucketRef = useRef(0), lastWheelTimeRef = useRef<number | null>(null), wheelDeltaRef = useRef(0), wheelRafRef = useRef<number | null>(null), pendingIndexRef = useRef<number | null>(null), pendingTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null), rafIdRef = useRef<number | null>(null), isInteractingRef = useRef(false)
  const onInteractStartRef = useRef(onInteractStart); onInteractStartRef.current = onInteractStart; const onInteractEndRef = useRef(onInteractEnd); onInteractEndRef.current = onInteractEnd
  ...
  const stopRaf = useCallback(() => { if (rafIdRef.current != null && typeof cancelAnimationFrame !== 'undefined') { cancelAnimationFrame(rafIdRef.current); rafIdRef.current = null }; if (wheelRafRef.current != null && typeof cancelAnimationFrame !== 'undefined') { cancelAnimationFrame(wheelRafRef.current); wheelRafRef.current = null } }, [])
  ...
  const setVelocityBucket = useCallback((velocity: number) => { const next = getVelocityBucket(velocity); if (next !== webVelocityBucketRef.current) { webVelocityBucketRef.current = next; setWebVelocityBucket(next) } }, [])
  const updateWheelVelocity = useCallback((delta: number) => { const now = Date.now(); const last = lastWheelTimeRef.current; if (last != null) { const dt = Math.max(1, now - last); setVelocityBucket(delta / dt) }; lastWheelTimeRef.current = now }, [setVelocityBucket])
  useEffect(() => { if (!isWeb) return; clearPendingTimer(); pendingIndexRef.current = null; setWebTransition(0); const next = indexToOffset(safeSelIdx, itemHeight); webOffsetRef.current = next; setWebOffset(next) }, [clearPendingTimer, isWeb, itemHeight, safeSelIdx, setWebTransition])
  const finalizePendingChange = useCallback(() => { if (readOnly) return; const nextIdx = pendingIndexRef.current; if (nextIdx == null) return; pendingIndexRef.current = null; clearPendingTimer(); setWebTransition(0); notifyEnd(); onChange(nextIdx) }, [clearPendingTimer, onChange, readOnly, setWebTransition])
  const startWebSnap = useCallback((targetIdx: number) => { if (readOnly) return; notifyStart(); const clampedIdx = clamp(targetIdx, 0, maxIdx); const targetOff = indexToOffset(clampedIdx, itemHeight); clearPendingTimer(); pendingIndexRef.current = clampedIdx; webOffsetRef.current = targetOff; setWebTransition(swipeDuration); setWebOffset(targetOff); swipeDuration <= 0 ? finalizePendingChange() : (pendingTimerRef.current = setTimeout(finalizePendingChange, swipeDuration + 80)) }, [clearPendingTimer, finalizePendingChange, itemHeight, maxIdx, readOnly, swipeDuration])
  const handleWheel = useCallback((event: { nativeEvent?: { deltaY?: number } }) => { if (readOnly) return; const delta = event.nativeEvent?.deltaY ?? 0; if (!delta) return; wheelDeltaRef.current += delta; if (wheelRafRef.current != null || typeof requestAnimationFrame === 'undefined') return; wheelRafRef.current = requestAnimationFrame(() => { wheelRafRef.current = null; const queued = wheelDeltaRef.current; wheelDeltaRef.current = 0; if (!queued) return; updateWheelVelocity(queued); const direction = queued > 0 ? 1 : -1; const { index } = offsetToIndex(webOffsetRef.current, itemHeight, total, data); const nextIdx = clamp(index + direction, 0, maxIdx); startWebSnap(nextIdx) }) }, [data, itemHeight, maxIdx, readOnly, startWebSnap, total, updateWheelVelocity])
  const webIdx = clamp(Math.round(-webOffset / itemHeight), 0, maxIdx)
  const webRender = useMemo(() => {
    if (!isWeb || total <= 0) return { items: null as React.ReactNode, topSpacer: null as React.ReactNode, bottomSpacer: null as React.ReactNode }
    let startIdx = 0; let endIdx = maxIdx
    if (webVirtualEnabled) { const baseBuffer = Math.max(visibleCnt * 2, 8); const velocityBoost = webVelocityBucket === 2 ? visibleCnt * 4 : webVelocityBucket === 1 ? visibleCnt * 2 : 0; const buffer = Math.min(baseBuffer + velocityBoost, Math.max(visibleCnt * 6, 24)); startIdx = clamp(webIdx - buffer, 0, maxIdx); endIdx = clamp(webIdx + buffer, 0, maxIdx) }
    const items: React.ReactNode[] = []; for (let index = startIdx; index <= endIdx; index += 1) { const item = data[index]; if (!item) continue; items.push(<WheelPickerItem key={`${index}-${String(item.value ?? '')}`} item={item} index={index} itemHeight={itemHeight} active={index === safeSelIdx} disabled={!!item.disabled} renderItem={renderItem} />) }
    const topH = startIdx * itemHeight; const bottomH = (maxIdx - endIdx) * itemHeight; return { items, topSpacer: topH > 0 && <View style={{ height: topH }} />, bottomSpacer: bottomH > 0 && <View style={{ height: bottomH }} /> }
  ...
  const handleWebTransitionEnd = useCallback((event: { nativeEvent?: { propertyName?: string } } & { propertyName?: string }) => { const propertyName = event.nativeEvent?.propertyName ?? event.propertyName; if (propertyName && propertyName !== 'transform' && propertyName !== 'webkitTransform') return; finalizePendingChange() }, [finalizePendingChange])
  const panResponder = useMemo(() => PanResponder.create({ onStartShouldSetPanResponder: () => !readOnly, onMoveShouldSetPanResponder: () => !readOnly, onPanResponderGrant: () => { stopRaf(); pendingIndexRef.current = null; notifyStart(); setWebTransition(0); startOffsetRef.current = webOffsetRef.current; startTimeRef.current = Date.now() }, onPanResponderMove: (_, gesture) => { if (readOnly) return; setVelocityBucket(gesture.vy); const next = clamp(startOffsetRef.current + gesture.dy, minOff, 0); webOffsetRef.current = next; typeof requestAnimationFrame === 'undefined' ? (setWebOffset(next), undefined) : (rafIdRef.current != null ? undefined : (rafIdRef.current = requestAnimationFrame(() => { rafIdRef.current = null; setWebOffset(webOffsetRef.current) }))) }, onPanResponderRelease: (_, gesture) => { if (readOnly) return; setVelocityBucket(0); const duration = Date.now() - startTimeRef.current; const distance = gesture.dy; let target = clamp(startOffsetRef.current + distance, minOff, 0); if (shouldMomentum(distance, duration)) target = momentumTarget(distance, duration, startOffsetRef.current, itemHeight, minOff); const { index } = offsetToIndex(target, itemHeight, total, data); startWebSnap(index) }, onPanResponderTerminationRequest: () => false, onPanResponderTerminate: () => { notifyEnd(); setWebTransition(0) } }), [data, itemHeight, minOff, notifyEnd, notifyStart, readOnly, setVelocityBucket, startWebSnap, stopRaf, total])
  const shouldCapture = !readOnly; const handleResponderCapture = useCallback(() => shouldCapture, [shouldCapture])
  ...
  const handleScrollEndDrag = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>) => { if (readOnly) return; const y = e.nativeEvent.contentOffset.y; lastOffsetRef.current = y; clearDragEndTimer(); dragEndTimerRef.current = setTimeout(() => { if (!momentumRef.current) { emitIdx(lastOffsetRef.current, true); notifyEnd() } }, 80) }, [clearDragEndTimer, emitIdx, notifyEnd, readOnly])
  ...
  const onMomEnd = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>) => { momentumRef.current = false; clearDragEndTimer(); const y = e.nativeEvent.contentOffset.y; lastOffsetRef.current = y; emitIdx(y, false); notifyEnd() }, [clearDragEndTimer, emitIdx, notifyEnd])
  if (isWeb) return (<View style={[W.column, { height: containerHeight }, W.grab]} {...({ onWheel: handleWheel } as unknown as React.ComponentProps<typeof View>)} {...panResponder.panHandlers}><View style={indicatorStyle} pointerEvents="none"><View style={createHairlineView({ position: 'top', color: indicatorColor, left: 0, right: 0 })} /><View style={createHairlineView({ position: 'bottom', color: indicatorColor, left: 0, right: 0 })} /></View><View style={[webTransform, webTransitionStyle]} {...({ onTransitionEnd: handleWebTransitionEnd } as unknown as React.ComponentProps<typeof View>)}><Spacer />{webRender.topSpacer}{webRender.items}{webRender.bottomSpacer}<Spacer /></View></View>)
  return (<View style={[W.column, { height: containerHeight }]} collapsable={false}><View style={indicatorStyle} pointerEvents="none"><View style={createHairlineView({ position: 'top', color: indicatorColor, left: 0, right: 0 })} /><View style={createHairlineView({ position: 'bottom', color: indicatorColor, left: 0, right: 0 })} /></View><ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} scrollEventThrottle={effectiveScrollThrottle} decelerationRate={decelerationRate} snapToInterval={itemHeight} snapToAlignment="start" bounces={false} overScrollMode="never" nestedScrollEnabled contentContainerStyle={nativeContainerStyle} onStartShouldSetResponderCapture={handleResponderCapture} onMoveShouldSetResponderCapture={handleResponderCapture} onScroll={handleScroll} onScrollBeginDrag={onDragStart} onScrollEndDrag={handleScrollEndDrag} onMomentumScrollBegin={handleMomentumScrollBegin} onMomentumScrollEnd={onMomEnd} scrollEnabled={!readOnly}>{data.map((item, index) => (<WheelPickerItem key={`${index}-${String(item.value ?? '')}`} item={item} index={index} itemHeight={itemHeight} active={index === safeSelIdx} disabled={!!item.disabled} renderItem={renderItem} />))}</ScrollView></View>)
}
...
const GRADIENT_OVERLAY_ALPHA = 0.25; const GRADIENT_STEPS = [0.95, 0.75, 0.55, 0.35]; const GRADIENT_STEPS_REVERSED = [0.35, 0.55, 0.75, 0.95]
...
const GradientMask: React.FC<{ height: number; color: string; position: 'top' | 'bottom'; maskType: NonNullable<PickerProps['maskType']> }> = ({ height, color, position, maskType }) => {
  const baseStyle = [S.gMask, { height }, position === 'top' ? { top: 0 } : { bottom: 0 }]; const overlayColor = withAlpha(color, GRADIENT_OVERLAY_ALPHA)
...
const PickerImpl: React.FC<PickerProps> = props => {
  ...
  const toolbar = showToolbar ? (<View style={[S.toolbar, { height: tokens.spacing.toolbarHeight, paddingHorizontal: tokens.spacing.actionPadding }]}><Pressable onPress={onCancel} accessibilityRole="button">{renderActionContent(cancelButtonText, tokens.colors.cancel)}</Pressable>{renderTitleContent(title)}<Pressable onPress={handleConfirm} accessibilityRole="button">{renderActionContent(confirmButtonText, tokens.colors.confirm)}</Pressable><View style={createHairlineView({ position: 'bottom', color: tokens.colors.indicator, left: 0, right: 0 })} /></View>) : null
...
const S = StyleSheet.create({ body: { position: 'relative', overflow: 'hidden' }, columns: { flex: 1, flexDirection: 'row' }, optTxt: { includeFontPadding: false }, indicator: { position: 'absolute', left: 0, right: 0, zIndex: 3 }, gMask: { position: 'absolute', left: 0, right: 0, zIndex: 2 }, toolbar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, title: { flex: 1, textAlign: 'center' }, ttlW: { flex: 1, alignItems: 'center', justifyContent: 'center' }, actTxt: { minWidth: 44, textAlign: 'center' }, actW: { minWidth: 44, alignItems: 'center', justifyContent: 'center' }, loading: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center' } })
