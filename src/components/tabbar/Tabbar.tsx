...
const TabbarBaseImpl: React.FC<TabbarProps> = props => {
  ...
  const barStyle = [styles.bar, { backgroundColor: background, paddingHorizontal: tokens.layout.paddingHorizontal, minHeight: tokens.layout.height }, border ? createHairlineBorderTop(tokens.colors.border) : null, contentStyle]
  ...
  return (
    <>
      ...
      <View {...rest} style={containerStyle} onLayout={handleLayout}>
        <ContentWrapper style={barStyle}>
          <TabbarContext.Provider value={contextValue}>
            <View style={rowStyle} accessibilityRole="tablist">{clonedChildren}</View>
