...
const InternalFormImpl = (props: FormProps, ref: React.ForwardedRef<FormInstance>) => {
  ...
  const defaultValuesRef = useRef<Record<string, unknown>>({}); const errorsRef = useRef<Record<string, string[]>>({}); const lastInitialValuesRef = useRef<Record<string, unknown>>(initialValues ?? defaultValuesRef.current); const fieldsRef = useRef<Record<string, RegisteredFieldOptions & { name: NamePath }>>({}); const dependencyGraphRef = useRef(new Map<string, Set<string>>()); const valuesRef = useRef<Record<string, unknown>>(initialValues ?? defaultValuesRef.current); const validationSeqRef = useRef<Record<string, number>>({}); const subscribersRef = useRef(new Set<(changedValues: Record<string, unknown>, allValues: Record<string, unknown>) => void>()); const onValuesChangeRef = useRef(onValuesChange); const onFinishRef = useRef(onFinish); onValuesChangeRef.current = onValuesChange; onFinishRef.current = onFinish
  const mergedInitialValues = initialValues ?? defaultValuesRef.current; const [, setVersion] = useState(0)
  const notify = useCallback((changedValues: Record<string, unknown>, newValues: Record<string, unknown>) => { subscribersRef.current.forEach(l => l(changedValues, newValues)) }, [])
  useEffect(() => { if (shallowEqualObject(lastInitialValuesRef.current, mergedInitialValues)) return; lastInitialValuesRef.current = mergedInitialValues; valuesRef.current = mergedInitialValues; errorsRef.current = {}; setVersion(v => v + 1); notify({ [FORM_ALL_FIELDS_KEY]: true }, mergedInitialValues) }, [mergedInitialValues, notify])
  const setFieldErrors = useCallback((name: NamePath, newErrors: string[]) => { const key = serializeNamePath(name); const prevE = errorsRef.current[key]; if (!newErrors.length) { if (!prevE) return; const cl = { ...errorsRef.current }; delete cl[key]; errorsRef.current = cl; notify({ [key]: getValueByName(valuesRef.current, name) }, valuesRef.current); return }; if (prevE?.[0] === newErrors[0] && prevE.length === newErrors.length) return; errorsRef.current = { ...errorsRef.current, [key]: newErrors }; notify({ [key]: getValueByName(valuesRef.current, name) }, valuesRef.current) }, [notify])
  const registerField = useCallback((name: NamePath, opts: RegisteredFieldOptions) => { const key = serializeNamePath(name); const prev = fieldsRef.current[key]; if (prev?.dependencies?.length) { for (const dep of prev.dependencies) { const dk = serializeNamePath(dep); const s = dependencyGraphRef.current.get(dk); if (!s) continue; s.delete(key); if (!s.size) dependencyGraphRef.current.delete(dk) } }; fieldsRef.current[key] = { ...opts, name }; if (opts.dependencies?.length) { for (const dep of opts.dependencies) { const dk = serializeNamePath(dep); const s = dependencyGraphRef.current.get(dk); if (s) s.add(key); else dependencyGraphRef.current.set(dk, new Set([key])) } }; if (opts.initialValue !== undefined) { const exInit = getValueByName(lastInitialValuesRef.current, name); if (exInit === undefined) lastInitialValuesRef.current = setValueByName(lastInitialValuesRef.current, name, opts.initialValue); const exState = getValueByName(valuesRef.current, name); if (exState === undefined) { const next = setValueByName(valuesRef.current, name, opts.initialValue); valuesRef.current = next; notify({ [key]: opts.initialValue }, next) } }; return () => { const cur = fieldsRef.current[key]; delete fieldsRef.current[key]; if (cur?.dependencies?.length) { for (const dep of cur.dependencies) { const dk = serializeNamePath(dep); const s = dependencyGraphRef.current.get(dk); if (!s) continue; s.delete(key); if (!s.size) dependencyGraphRef.current.delete(dk) } }; setFieldErrors(name, []) } }, [notify, setFieldErrors])
  const localeRef = useRef(locale); localeRef.current = locale
  const validateField = useCallback(async (name: NamePath, trigger?: string, valueOverride?: unknown, valuesOverride?: Record<string, unknown>): Promise<boolean> => { const key = serializeNamePath(name); const seq = (validationSeqRef.current[key] ?? 0) + 1; validationSeqRef.current[key] = seq; const fOpts = fieldsRef.current[key]; const fRules = fOpts?.rules ?? []; if (!fRules.length) { setFieldErrors(name, []); return true }; let appRules = trigger ? fRules.filter(r => { const rt = r.validateTrigger ?? fOpts.validateTrigger; return !rt || normalizeTrigger(rt).includes(trigger) }) : fRules; if (!appRules.length) { if (!errorsRef.current[key]?.length) return true; appRules = fRules }; const curVals = valuesOverride ?? valuesRef.current; const val = valueOverride ?? getValueByName(curVals, name); const fbMsg = localeRef.current?.vanForm?.validationFailed; for (const rule of appRules) { const result = runRuleValidation(rule, val, curVals, fbMsg); const err = isPromiseLike(result) ? await result : result; if (validationSeqRef.current[key] !== seq) return true; if (err) { setFieldErrors(name, [err]); return false } }; if (validationSeqRef.current[key] !== seq) return true; setFieldErrors(name, []); return true }, [setFieldErrors])
  ...
  const formApi = useMemo((): FormInstance => ({ submit: async () => { try { const res = await validateFields(); onFinishRef.current?.(res); return res } catch { return undefined } }, getFieldsValue: () => valuesRef.current, setFieldsValue: (next, opts) => { const sV = opts?.validate ?? false; const prev = valuesRef.current; let m = prev; const ch: Record<string, unknown> = {}; Object.keys(next).forEach(k => { const nv = next[k]; if (getValueByName(m, k) === nv) return; ch[k] = nv; m = setValueByName(m, k, nv); onValuesChangeRef.current?.(m, k, nv); if (sV) validateWithDependencies(k, k, undefined, nv, m) }); if (m === prev) return; valuesRef.current = m; notify(ch, m) }, resetFields: () => { let n = lastInitialValuesRef.current; Object.values(fieldsRef.current).forEach(mem => { if (mem.initialValue === undefined) return; if (getValueByName(n, mem.name) !== undefined) return; n = setValueByName(n, mem.name, mem.initialValue) }); valuesRef.current = n; errorsRef.current = {}; notify({ [FORM_ALL_FIELDS_KEY]: true }, n) }, validateFields, getFieldError: (name: NamePath) => errorsRef.current[serializeNamePath(name)] ?? [] }), [validateFields, validateWithDependencies, notify])
