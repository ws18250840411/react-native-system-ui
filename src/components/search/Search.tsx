...
const SearchComponent = (props: SearchProps, ref: React.Ref<SearchRef>) => {
  ...
  const labelNode = !label ? null : isText(label) ? <Text style={{ marginRight: tokens.spacing.labelGap, color: tokens.colors.label, fontSize: tokens.typography.label, fontWeight: tokens.typography.labelWeight }}>{label}</Text> : <View style={{ marginRight: tokens.spacing.labelGap }}>{label}</View>

  const actionNode = action ? <View style={[styles.actionWrapper, { marginLeft: tokens.spacing.actionGap }]}>{action}</View> : !shouldShowAction ? null : isCustomActionText ? <View style={[styles.actionWrapper, { marginLeft: tokens.spacing.actionGap }]}>{actionText}</View> : <Pressable style={[styles.actionWrapper, { marginLeft: tokens.spacing.actionGap, opacity: cancelActionPress.states.pressed ? tokens.opacity.actionPressed : 1 }]} {...cancelActionPress.interactionProps}><Text style={{ color: tokens.colors.action, fontSize: tokens.typography.action, fontWeight: tokens.typography.actionWeight }}>{actionText ?? locale.cancel}</Text></Pressable>
