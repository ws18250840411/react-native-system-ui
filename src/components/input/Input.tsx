...
const InputImpl = (props: InputProps, ref: React.ForwardedRef<InputInstance>) => {
  const { type, keyboardType: keyboardTypeProp, align: alignProp, inputAlign: inputAlignProp, clearTrigger: clearTriggerProp, onChange, onChangeText, showWordLimit, style, inputStyle, fieldTokensOverride, tokensOverride, ...rest } = props
  ...
  const handleChangeText = useCallback((value: string) => { onChangeRef.current?.(value); onChangeTextRef.current?.(value) }, [])
  useImperativeHandle(ref, () => ({ focus: () => inputRef.current?.focus?.(), blur: () => inputRef.current?.blur?.(), clear: () => inputRef.current?.clear?.(), get nativeElement() { return inputRef.current?.nativeElement ?? null } }), [])
  ...
  const fieldStyle = [{ paddingHorizontal: tokens.spacing.paddingHorizontal, paddingVertical: tokens.spacing.paddingVertical, backgroundColor: tokens.colors.background }, style]
...
const TextAreaImpl = (props: InputTextAreaProps, ref: React.ForwardedRef<InputInstance>) => {
  const { autoSize: autoSizeProp, ...rest } = props
  ...
  const resolvedAutoSize: boolean | FieldAutosizeConfig | undefined = useMemo(() => !autoSizeProp || isBoolean(autoSizeProp) ? autoSizeProp : (() => {
