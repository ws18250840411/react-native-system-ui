...
const AreaImpl: React.FC<AreaProps> = props => {
  ...
  const cols = colP >= 1 && colP <= 3 ? colP : tokens.defaults.columnsNum; const { province_list, city_list, county_list } = areaList
  const columns = useMemo(() => buildAreaColumns({ province_list, city_list, county_list }, cols), [city_list, county_list, province_list, cols]); const normVal = useMemo(() => value === undefined ? undefined : normalizeCascadeValue(columns as PickerOption[], value, cols), [columns, cols, value]); const normDef = useMemo(() => defaultValue === undefined ? undefined : normalizeCascadeValue(columns as PickerOption[], defaultValue, cols), [columns, cols, defaultValue])
