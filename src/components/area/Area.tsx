  ...
  if (columnsNum === 3) {
    ...
    citiesByProvince.forEach(cityOptions => {
      cityOptions.forEach(cityOption => {
        ...
        if (counties && counties.length) cityOption.children = counties
      })
    })
  }
...
const AreaImpl: React.FC<AreaProps> = props => {
  ...
  const resolvedColumnsNum = useMemo(() => (columnsNum >= 1 && columnsNum <= 3 ? columnsNum : tokens.defaults.columnsNum), [columnsNum])
  ...
  const wrappedOnChange = useCallback((values: PickerValue[], options: (PickerOption | undefined)[]) => {
    onChangeRef.current?.(values.map(String), options as (AreaOption | undefined)[])
  }, [])
  const wrappedOnConfirm = useCallback((values: PickerValue[], options: (PickerOption | undefined)[]) => {
    onConfirmRef.current?.(values.map(String), options as (AreaOption | undefined)[])
  }, [])

  return <Picker {...pickerProps} columns={columns} interactionMode={interactionMode} value={normalizedValue} defaultValue={normalizedDefaultValue} onChange={onChange ? wrappedOnChange : undefined} onConfirm={onConfirm ? wrappedOnConfirm : undefined} />
}
