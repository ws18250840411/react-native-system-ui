...
const CheckboxImpl = (p: CheckboxProps, ref: React.ForwardedRef<View>) => {
  ...
  const { inputProps: gip } = useCheckboxGroupItem({ ...cr, value: sv ?? '', isDisabled: rd, 'aria-label': ral }, ig && g ? g.state : EMPTY_CHECKBOX_GROUP_STATE, ar)
  const { inputProps: sip } = useCheckbox({ ...cr, isDisabled: rd, value: sv, 'aria-label': ral }, ss, ar)
  const ip: Partial<React.ComponentProps<typeof Pressable>> = ig && g ? (gip as unknown as Partial<React.ComponentProps<typeof Pressable>>) : (sip as unknown as Partial<React.ComponentProps<typeof Pressable>>)
  ...
  const mip: Partial<React.ComponentProps<typeof Pressable>> = ip ? { ...ip, onPress: (e: GestureResponderEvent) => { onClick?.(e); if (ig && g?.max && !ic && g.state.value.length >= g.max) return; if (oop) { oop(e); return }; if (ig && g && sv !== undefined) { ic ? g.state.removeValue(sv) : g.state.addValue(sv); return }; if (p.checked !== undefined) { onChange?.(!p.checked); return }; if (onChange) ss.setSelected(!ss.isSelected) } } : {}
  const ln = !isRenderable(children) ? null : isText(children) ? <Text accessible={false} style={[t.layout.label, { color: lc, fontSize: t.typography.fontSize, lineHeight: t.typography.fontSize * t.typography.lineHeightMultiplier, fontFamily: t.typography.fontFamily, fontWeight: t.typography.fontWeight }, labelStyle]}>{children}</Text> : <View accessible={false} style={labelStyle as unknown as StyleProp<ViewStyle>}>{children}</View>
  ...
  return i ? <Pressable {...mip} ref={ir} disabled={rd} accessibilityLabel={ral} accessibilityRole="checkbox" accessibilityState={{ checked: ic, disabled: !!rd }} style={({ pressed }) => [t.layout.container, style, Platform.OS === 'web' && ({ cursor: 'pointer' } as unknown as ViewStyle), pressed && { opacity: 0.8 }]} hitSlop={hitSlop}>{cnt}</Pressable> : <View style={[t.layout.container, style]}>{cnt}</View>
}
