...
import { useAccessibilityAnnouncement, useAutoClose, useLatestRef, useVisibilityLifecycle } from '../../internal/feedback'
...
const ToastContentImpl: React.FC<ToastProps> = props => {
  ...
  const handleClose = React.useCallback(() => { callbacksRef.current.onClose?.() }, [callbacksRef]); const pressProps = useAriaPress({ disabled: !closeOnClick, onPress: handleClose, extraProps: { accessibilityRole: closeOnClick ? 'button' : 'alert', accessibilityHint: closeOnClick ? (locale?.vanToast?.closeHint ?? 'Double-tap to dismiss') : undefined, accessibilityLiveRegion: 'assertive' } })
