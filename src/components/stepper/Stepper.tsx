...
const StepperImpl = (p: StepperProps, ref: React.ForwardedRef<StepperInstance>) => {
  ...
  const gBS = useCallback((type: 'plus' | 'minus', state: PressableStateCallbackType) => { const isP = type === 'plus'; const dS = isP ? pD : mD; if (theme === 'round') { const base = [S.btn, bBS, { borderRadius: bS / 2, opacity: dS ? t.opacity.roundDisabled : 1 }] as const; if (isP) { return [...base, { backgroundColor: t.colors.roundTheme, ...(state.pressed && !dS && { opacity: t.opacity.pressed }) }, buttonStyle] }; return [...base, { backgroundColor: t.colors.roundThemeBackground, borderWidth: StyleSheet.hairlineWidth, borderColor: t.colors.roundTheme, ...(state.pressed && !dS && { opacity: t.opacity.pressed }) }, buttonStyle] }; const cornerStyle = type === 'minus' ? { borderTopLeftRadius: r, borderBottomLeftRadius: r } : { borderTopRightRadius: r, borderBottomRightRadius: r }; const bg = dS ? t.colors.buttonDisabledBackground : state.pressed ? t.colors.active : t.colors.background; return [S.btn, bBS, cornerStyle, { backgroundColor: bg }, buttonStyle] }, [bBS, buttonStyle, mD, pD, r, bS, theme, t.colors.active, t.colors.background, t.colors.buttonDisabledBackground, t.colors.roundTheme, t.colors.roundThemeBackground, t.opacity.pressed, t.opacity.roundDisabled])
