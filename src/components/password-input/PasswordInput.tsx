...
const PasswordInputImpl = (props: PasswordInputProps, ref: React.ForwardedRef<PasswordInputRef>) => {
  ...
  const wrapperStyle = [S.w, { backgroundColor, borderRadius: radii.wrapper, paddingHorizontal: spacing.none, opacity: disabled ? opacity.disabled : 1 }]
  const sheetStyle = [S.s, { borderRadius: hasGutter ? 0 : radii.wrapper, backgroundColor }]
  return <View style={style}><Pressable {...rest} style={wrapperStyle} onPress={focusInput} disabled={disabled} accessibilityRole="button" accessibilityState={{ disabled }}><View style={sheetStyle}>{cells.map((item, i) => { const filledTextStyle = [cellTextBase, cellTextStyle, !mask && item.isFilled && highlightTextStyle]; const baseCell = [S.c, { backgroundColor: colors.background, height: sizing.cellHeight }, cellStyle, item.isFilled && cellFilledStyle]; if (hasGutter) { baseCell.push(S.cg, { borderRadius: radii.cellGutter }, i > 0 && { marginLeft: gutterValue }) }; return <View key={item.key} style={baseCell}>{mask ? <View style={[{ width: sizing.maskSize, height: sizing.maskSize, borderRadius: sizing.maskSize / 2, backgroundColor: colors.text, opacity: item.isFilled ? 1 : 0 }, maskStyle]} /> : <Text style={filledTextStyle} numberOfLines={1}>{item.char ?? ''}</Text>}{item.showBlink ? <View testID="password-input-cursor" style={[S.cr, { width: sizing.cursorWidth, height: `${sizing.cursorHeightRatio * 100}%`, borderRadius: sizing.cursorWidth / 2, top: `${sizing.cursorTopRatio * 100}%`, marginLeft: -sizing.cursorWidth / 2, backgroundColor: colors.cursor, opacity: cursorVisible ? 1 : 0 }, cursorStyle]} /> : null}{hasGutter && <View style={createHairlineView({ position: 'all', color: colors.border, borderRadius: radii.cellGutter })} />}{!hasGutter && i < lengthValue - 1 && <View style={createHairlineView({ position: 'right', color: colors.border, top: 0, bottom: 0 })} />}</View>})}<TextInput ref={inputRef} value={normalizedCode} editable={!disabled} keyboardType={keyboardType} inputMode={inputMode} maxLength={lengthValue} autoFocus={false} secureTextEntry={mask} {...hiddenInputProps} style={[S.hi, { opacity: opacity.hidden }]} onChangeText={handleChangeText} onFocus={handleFocus} onBlur={handleBlur} accessible={false} /></View>{!hasGutter && <View style={createHairlineView({ position: 'all', color: colors.border, borderRadius: radii.wrapper })} />}</Pressable>{tipInfo ? <View style={[S.iw, { marginTop: spacing.infoMarginTop }]}>{isText(tipInfo) ? <Text style={[S.it, { color: tipColor }]}>{tipInfo}</Text> : tipInfo}</View> : null}</View>
}
